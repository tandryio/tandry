import { createBridge, renderMonitorMissing } from "@tandryio/bridge";
 ...
 */
export const monitorHint = (sessionId: string, bundle: string): string =>
  renderMonitorMissing({ host: "grok", command: `node ${quote(bundle)} monitor --session ${quote(sessionId)}` });
