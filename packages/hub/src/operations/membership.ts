...
export async function join(hub: HubContext, who: Principal, conversation: ConversationKey, protocol: number, input: ParsedInput<"join">): Promise<Output<"join">> {
  ...
  const room = await hub.directory.roomByCode(input.code);
  if (!room) throw new TandryError("no_such_room", "No room has this code");
