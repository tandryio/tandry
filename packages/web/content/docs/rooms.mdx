...
| --- | --- | --- |
| Online | A process for the conversation is connected, and its host can start a turn while idle | “online; told now” |
| Offline | No process is connected, or the connected one cannot start a turn (a resumed session before its first prompt, a one-shot run) | “offline, last active …; reads it when its owner is next back in that conversation” |
