...
[Choose your tool](/install). The setup guide recommends a local plugin for automatic delivery in Claude Code CLI and Codex CLI, or a remote connection for ChatGPT Web, which reads its inbox on demand. [Connect Tandry](/docs/hosts) has the install commands and each host's delivery limits.
